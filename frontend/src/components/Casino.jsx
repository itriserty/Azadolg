...
import JackpotTournament from './JackpotTournament';

export default function Casino({ user, onUpdateUser }) {
  const [jackpot, setJackpot] = useState(0);
  const [activeSubTab, setActiveSubTab] = useState('cases'); // 'cases' | 'duels' | 'tournament'
    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="relative overflow-hidden bg-gradient-to-br from-[#241a0d] via-[#0d1715] to-black border-2 border-amber-400 shadow-[0_0_30px_rgba(245,158,11,0.25)] rounded-3xl p-6 text-center">
        ...
        <div className="text-amber-400 font-extrabold text-[10px] uppercase tracking-widest flex items-center justify-center gap-1 mb-2">
          <Sparkles className="w-3.5 h-3.5 animate-pulse" /> Призовой Фонд Джекпота <Sparkles className="w-3.5 h-3.5 animate-pulse" />
        </div>
        ...
        <p className="text-[10px] text-gray-400 max-w-md mx-auto mt-2 leading-relaxed">
          Накопительный призовой фонд разыгрывается в 6-игроковом Турнире (Группы + Плей-офф)! 🚀
        </p>
      </div>

      {/* Переключатель вкладок внутри Казино */}
      <div className="flex bg-[#0d1715]/80 border border-gray-800 p-1.5 rounded-2xl gap-1.5 max-w-md mx-auto">
        <button
        ...
        <button
          ...
          className={`flex-1 py-2 rounded-xl text-xs font-bold transition-all ${
            activeSubTab === 'tournament'
              ? 'bg-gradient-to-r from-amber-500/20 to-emerald-500/20 border border-amber-500/40 text-amber-300 font-extrabold shadow'
              : 'text-gray-400 hover:text-gray-250 border border-transparent'
          }`}
        >
          🏆 Турнир
      ...
      <div>
        {activeSubTab === 'cases' ? (
          ...
          <JackpotTournament currentUser={user} />
        )}
      </div>
